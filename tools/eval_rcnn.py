...
from lib.datasets.argo_rcnn_dataset import ArgoRCNNDataset
    ...
    for data in dataloader:
        ...
        for bs_idx in range(batch_size):
            ...
            if not args.test:
                ...
                output_data = np.concatenate((np.dot(kitti_to_argo, cur_pts_rect.T).T, cur_gt_cls.reshape(-1, 1), cur_pred_cls.reshape(-1, 1)), axis=1)

            else:
                output_data =  np.concatenate((np.dot(kitti_to_argo, cur_pts_rect.T).T, cur_pred_cls.reshape(-1, 1)), axis=1)
...
def create_dataloader(logger):
    ...
    # create dataloader
    test_set = ArgoRCNNDataset(root_dir=DATA_PATH, npoints=cfg.RPN.NUM_POINTS, split=cfg.TEST.SPLIT, mode=mode,
